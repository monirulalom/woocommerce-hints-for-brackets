...
define(function (require, exports, module) {
    ...
    var AppInit = brackets.getModule('utils/AppInit'),
        WooCommercefuncHint = require('text!Woofunc.txt');
     ...
     */
    WooCommercehints.prototype.hasHints = function (editor, implicitChar) {
        this.editor = editor;
        var cursor = this.editor.getCursorPos();
        
        // if it is not the same line as the last input - rebuild word list
        if(cursor.line != this.lastLine){
            var rawWordList = WooCommercefuncHint.match(this.tokenDefinition);
            this.cachedWordList = [];
     ...
     */
    WooCommercehints.prototype.getHints = function (implicitChar) {
        var cursor = this.editor.getCursorPos();
        var lineBeginning = {line:cursor.line,ch:0};
        var textBeforeCursor = this.editor.document.getRange(lineBeginning, cursor);
        var symbolBeforeCursorArray = textBeforeCursor.match(this.currentTokenDefinition);
     ...
     */
    WooCommercehints.prototype.insertHint = function (hint) {
        var cursor = this.editor.getCursorPos();
        var lineBeginning = {line:cursor.line,ch:0};
        var textBeforeCursor = this.editor.document.getRange(lineBeginning, cursor);
    ...
    AppInit.appReady(function () {
        var WooCommerceHints = new WooCommercehints();
        CodeHintManager.registerHintProvider(WooCommerceHints, ["php"], 10);
    });
});
